...
ECLdatabase<-function(fname,year,fout=NA)
{
  ...
  ##Identify if in the parallel coastal region, for reference!
  I<-which(fixes[,6]>=144 & fixes[,6]<149 & fixes[,7]<(-37) & fixes[,7]>=-41)
  fixes[I,14]<-2
  I<-which(fixes[,6]<(149+(37+fixes[,7])/2) & fixes[,6]>=(144+(37+fixes[,7])/2) & fixes[,7]<(-31) & fixes[,7]>=-37)
  fixes[I,14]<-2
  I<-which(fixes[,6]<152 & fixes[,6]>147 & fixes[,7]<=(-24) & fixes[,7]>=-31)
  fixes[I,14]<-2
  ...
  for(i in 1:length(events[,1])) 
  {
    ...
    if(length(which(fixes[I,14]==1))>0) events[i,9]=1 ##Ever in ECL region
    if(length(which(fixes[I,14]==2))>0 & events[i,9]==0) events[i,9]=2 ##Ever near coast
    events[i,10]<-min(fixes[I,5] %% 10) ##Ever closed  
  }
  
  ## Take only the events where at least one fix is closed & in the ECL OR ADJACENT region
  ...
  Events<-data.frame(cbind(events2[,1:6],events2[,9]))
  names(Events)=c('ID','Length','Date1','Date2','MSLP','CV','Location')
  ...
  if(is.na(fout)) fout=fname
  outF=paste('ECLfixes_',fout,'.csv',sep="")
  outE=paste('ECLevents_',fout,'.csv',sep="")
