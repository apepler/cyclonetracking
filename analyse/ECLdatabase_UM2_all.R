...
ECLdatabase<-function(yearS,yearE,output)
{
  ...
  read.table(fname1[1], sep="",skip=0)->fixes  
  I=which(fixes[,3]<(yearS %% 100)*10000)
  fixesDEC=fixes[I,]
  I<-which(fixes[,3]>=(yearS %% 100)*10000 & fixes[,3]<((yearS+1) %% 100)*10000)
  fixes=fixes[I,]
  
  for(i in 2:length(year))
  {
    read.table(fname1[i], sep="",skip=0)->data
    if(year[i]==1999 | year[i]==2000) yy=year[i]-1980 else yy=year[i]
    I=which(data[,3]<(yy %% 100)*10000)
    fixesDEC=rbind(fixesDEC,data[I,])
    I<-which(data[,3]>=(yy %% 100)*10000 & data[,3]<((yy+1) %% 100)*10000)
  ...
  #Fix dates
  date1=fixes[,3]
  I=which(date1<800101)
  fixes[I,3]=date1[I]+20000000
  I=which(date1>=800101)
  fixes[I,3]=date1[I]+19000000
...
rm(fixesDEC)
  ...
  I<-which(events2[,2]>1 & events2[,10]==0)
  events3<-events2[I,]
