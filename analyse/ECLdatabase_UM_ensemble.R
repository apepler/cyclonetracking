...
##Assumes a filename of track_year_member.dat

ECLdatabase<-function(yearS,yearE,output,member,init=F)
{

  ##Files with ECL track data for required years & preceding summers.  
  year<-seq(yearS,yearE)
  fname1=paste('tracks_',as.character(year),'_',member,'.dat',sep="")
...
rm(fixesDEC)
  ...
  outF=paste('ECLfixes_',output,'_',member,'.csv',sep="")
  outE=paste('ECLevents_',output,'_',member,'.csv',sep="")
